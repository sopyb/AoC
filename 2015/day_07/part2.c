...
int main(void) {
    ...
    int initialA = value(getInstructionByWire("a"));

    memset(circuitWireValues, 0, sizeof(circuitWireValues));

    for (int i = 0; i < 1000; i++) {
        if (instructions[i].result == computeWireHash("b")) {
            sprintf(instructions[i].leftOperand, "%d", initialA);
